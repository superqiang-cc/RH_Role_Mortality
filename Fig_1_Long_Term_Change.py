...
def plot_trend(to_show, sig, cmp, nrm, tt, od, s=0.5):

    # mask the ocean
    to_show_mask = maskoceans(lon_mesh, lat_mesh, to_show, inlands=True, resolution='c')
    ...
    if isinstance(sig, (int, float)):
        lon_mesh_sig[s < 0.5] = np.NaN
        lat_mesh_sig[s < 0.5] = np.NaN
        s[s < 0.5] = np.NaN
        s = maskoceans(lon_mesh, lat_mesh, s, inlands=True, resolution='c')
        s = s[40:600:10, ::10]
    else:
    ...
    lon_mesh_sig = maskoceans(lon_mesh, lat_mesh, lon_mesh_sig, inlands=True, resolution='c')
    lat_mesh_sig = maskoceans(lon_mesh, lat_mesh, lat_mesh_sig, inlands=True, resolution='c')
    lon_mesh_sig.fill_value = np.NaN
    lat_mesh_sig.fill_value = np.NaN
    ...
    ax.add_feature(cfeature.OCEAN, facecolor=[0.7, 0.7, 0.7], zorder=3)
...
plot_trend(to_show, sig, cmap_t, norm_t, r'T$_{\mathrm{air}}$ trend', 'a')
...
plot_trend(to_show, sig, cmap_sh, norm_sh, 'Q trend', 'b')
...
plot_trend(to_show, sig, cmap_rh, norm_rh, 'RH trend', 'c')
...
plot_trend(to_show, np.NaN, cmap_cst, norm_cst, r'HSI trend (day$_{\mathrm{mean}}$)', 'd', sig_s)
...
sig_s[sig_s > 0.5] = 1
plot_trend(to_show, np.NaN, cmap_cst, norm_cst, r'HSI trend (day$_{\mathrm{max}}$)', 'e', sig_s)
