...
# load X (739, 12) and y (739) for random forest

X_org <- read.csv(file=paste0("Regional Variation in the Role of Humidity on City-level Heat-Related Mortality_TableS4_QG.csv"))
y_str_org <- X_org[, 14]  # BFI is in column 14

X_org <- X_org[, -14]  # remove column 14 (BFI)
X_org <- X_org[, -1]  # remove column 1 (City Name)
